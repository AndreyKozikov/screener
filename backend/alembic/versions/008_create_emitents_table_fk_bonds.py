"""Create emitents table and add FK from bonds.emitent_id to emitents.id.

Revision ID: 008
Revises: 007
Create Date: 2026-02-07

Таблица emitents: id, moex_id UNIQUE, inn UNIQUE, okpo, title.
bonds.emitent_id ссылается на emitents.id.
Существующие данные bonds сохраняются; emitent_id устанавливается в NULL
до первого refresh (приложение заполнит при сохранении).
"""
...
def upgrade() -> None:
    """Создаёт таблицу emitents и добавляет FK bonds.emitent_id -> emitents.id."""
    # 1. Создаём таблицу emitents
    op.create_table(
        "emitents",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("moex_id", sa.Integer(), nullable=True),
        sa.Column("inn", sa.String(length=32), nullable=True),
        sa.Column("okpo", sa.String(length=32), nullable=True),
        sa.Column("title", sa.String(), nullable=True),
        sa.UniqueConstraint("moex_id", name="uq_emitents_moex_id"),
        sa.UniqueConstraint("inn", name="uq_emitents_inn"),
    )

    # 2. SQLite не поддерживает ALTER ADD FOREIGN KEY. Пересоздаём bonds с FK.
...
def downgrade() -> None:
    """Откат: удаление FK, восстановление bonds без FK, удаление emitents."""
    op.drop_table("bondmarketdatayield")
    op.drop_table("bondmarketdata")
    op.drop_table("bondsecurity")
