...
SYSTEM_PROMPT = """Ты — система извлечения структурированных данных из текста. Твоя единственная задача — извлечь параметры ценных бумаг из предоставленного сообщения о существенном факте и вернуть результат в виде строго валидного JSON.
...
`registration_number`
Ищи фразу «регистрационный номер». Верни только сам номер.

`issue_registration_number`
Извлекай только если в тексте явно указан **отдельный** номер выпуска (в отличие от номера программы). Иначе — `null`.
...
`security_type`
Определи по контексту:
- «облигации» или «биржевые облигации» → `Облигация`
- «акции» → `Акция`
...
class EventProcessingService:
    """Сервис для конвейерной обработки событий и извлечения деталей через LLM."""
    ...
    def process_all_events(self, target_inn: Optional[str] = None) -> Dict[str, Any]:
        """Обходит файлы событий, отправляет в LLM и сохраняет результат.

        Args:
            target_inn: Файлы парсятся только для указанного ИНН, если задан.
