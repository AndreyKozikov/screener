...
«Отчетность МСФО», «Отчетность РСБУ», «Отчетность», «МСФО», «РСБУ» — такие имена
исключаются из списка до вызова API. Сохраняются только те markdown-файлы, в которых
...
def _filename_excluded_from_conversion(filename: str) -> bool:
    """True, если имя файла содержит одну из фраз отчётности (проверка без учёта регистра)."""
...
class PdfConversionService:
    ...
    def convert(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Исключаем файлы по имени (отчётность МСФО/РСБУ и т.д.) — проверка без учёта регистра.
        ...
        if excluded_count > 0:
            logger.info(
                "Исключено из конвертации по имени (отчётность МСФО/РСБУ и т.д.): %d файл(ов)",
                excluded_count,
            )
