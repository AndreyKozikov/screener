"""Сервис синхронизации истории биржевых торгов.

Обеспечивает загрузку и хранение ежедневных данных о торгах облигациями
через MOEX ISS API. Поддерживает инкрементальное обновление и параллельную
обработку большого списка ценных бумаг.
"""
...
class TradingHistoryService:
    """Сервис управления историческими данными торгов.

    Реализует сложную логику многопоточной загрузки истории из MOEX ISS,
    включая обработку пагинации, фильтрацию дубликатов и оптимизированную
    запись в специализированную базу данных истории.
    """
