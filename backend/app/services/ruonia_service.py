"""Сервис синхронизации и управления данными индикатора RUONIA.

Модуль реализует инкрементальную загрузку истории ставок RUONIA с сайта ЦБ РФ
(через экспорт в Excel), парсинг данных и их хранение в локальной базе данных.
"""
...
class RuoniaService:
    """Сервис для работы с данными RUONIA.

    Обеспечивает актуальность индикатора RUONIA в системе, выполняя загрузку
    новых значений и предоставляя аналитическую информацию в различных форматах (DTO, Markdown).

    Attributes:
        CBR_RUONIA_BASE_URL (str): URL для загрузки Excel-файла с динамикой ставок.
        DEFAULT_START_DATE (date): Дата начала исторического ряда.
        data_dir (Path): Директория для хранения временных файлов.
