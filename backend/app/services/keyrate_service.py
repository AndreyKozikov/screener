"""Сервис синхронизации и управления данными ключевой ставки ЦБ РФ.

Модуль реализует инкрементальную загрузку истории изменений ключевой ставки
с официального сайта Банка России, парсинг данных и их хранение в локальной БД.
"""
...
class KeyRateService:
    """Сервис для работы с ключевой ставкой ЦБ РФ.

    Обеспечивает актуальность данных ключевой ставки в системе путем
    периодической синхронизации с внешним источником (ЦБ РФ) и предоставляет
    удобный интерфейс для получения этих данных другими компонентами.

    Attributes:
        CBR_KEYRATE_URL (str): URL-адрес для загрузки данных с сайта ЦБ.
        DEFAULT_START_DATE (date): Точка отсчета для первичной загрузки (дата введения ставки).
        logger (Logger): Объект для логирования операций обновления.
    """
