...
# Промпты для каждого этапа (те же, что и для LLM и Qwen)
...
async def call_grok_with_data(
    ...
    Поддерживает reasoning mode для Grok 4.1 Fast.
    Возвращает ответ модели и обновленный список сообщений с reasoning_details.
    ...
    Args:
        ...
        previous_messages: Предыдущие сообщения с reasoning_details (для продолжения reasoning)
    
    Returns:
        tuple: (ответ модели, обновленный список сообщений)
    """
...
async def analyze_bonds_with_grok(
    ...
    """
    Analyze bonds using Grok 4.1 Fast via OpenRouter with multi-stage pipeline.
