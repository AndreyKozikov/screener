"""Роутеры для интеграции с Grok AI через OpenRouter.

Этот модуль содержит роутеры FastAPI для обработки HTTP запросов на анализ
облигаций с использованием модели Grok через OpenRouter API. Реализует
...
class GrokAnalysisResponse(BaseModel):
    """Модель ответа для анализа облигаций через Grok AI.
...
def get_openai_client() -> OpenAI:
    ...
    Настраивает таймаут 25 минут для длительных запросов к Grok.
    ...
    if openai_client is None:
        ...
        # Configure timeout: 25 minutes (1500 seconds) for long-running Grok requests
...
async def call_grok_with_data(
    ...
    """Вызывает Grok через OpenRouter с промптом и данными используя OpenAI клиент.
    
    Выполняет запрос к модели Grok через OpenRouter API с указанным промптом и данными.
    Поддерживает reasoning mode для Grok 4.1 Fast и продолжение reasoning между этапами
    через передачу предыдущих сообщений с reasoning_details.
    ...
    Args:
        ...
        model: Название модели для использования (по умолчанию "x-ai/grok-4.1-fast:free").
        ...
        previous_messages: Предыдущие сообщения с reasoning_details для продолжения
            reasoning между этапами. Если None, создается новый диалог.
    
    Returns:
        Кортеж из двух элементов:
        - Ответ модели (строка с результатом анализа)
        - Обновленный список сообщений с reasoning_details для использования
    ...
    Note:
        Reasoning mode включается через extra_body={"reasoning": {"enabled": True}}.
    ...
    if wait_before:
        print(f"[GROK] Waiting 60 seconds before {stage_name} (rate limit)...")
        await asyncio.sleep(60)
        print(f"[GROK] Wait complete, proceeding with {stage_name}")
...
Используй эти данные для анализа."""
    ...
    if previous_messages is None:
        ...
        # Используем предыдущие сообщения и добавляем новое
    ...
    print(f"\n{'='*80}")
    print(f"[GROK DEBUG] {stage_name}")
    print(f"{'='*80}")
    print(f"[GROK DEBUG] Model: {model}")
    print(f"[GROK DEBUG] Prompt length: {len(prompt)} chars")
    print(f"[GROK DEBUG] Data content length: {len(data_content)} chars")
    print(f"[GROK DEBUG] Full prompt length: {len(full_prompt)} chars")
    print(f"[GROK DEBUG] Messages count: {len(messages)}")
    print(f"\n[GROK DEBUG] Full prompt:")
    print(f"{full_prompt}")
    
    try:
        print(f"\n[GROK DEBUG] Request parameters:")
        print(f"  - model: {model}")
        print(f"  - max_tokens: 32000")
        print(f"  - reasoning enabled: True")
        print(f"  - messages count: {len(messages)}")
        print(f"\n[GROK DEBUG] Sending request to OpenRouter API via OpenAI client...")
        
        # Используем extra_body для включения reasoning mode для Grok
        ...
        if not completion.choices or len(completion.choices) == 0:
            print(f"\n[GROK DEBUG] ERROR: Model returned no choices")
            raise HTTPException(
                status_code=500,
        ...
        if not result:
            print(f"\n[GROK DEBUG] ERROR: Model returned empty response")
            raise HTTPException(
                status_code=500,
                detail="Модель не вернула ответ. Попробуйте позже."
            )
        
        # Сохраняем reasoning_details для последующих запросов
        ...
        print(f"\n[GROK DEBUG] Response received:")
        print(f"  - Response length: {len(result)} chars")
        print(f"  - Has reasoning_details: {hasattr(response_message, 'reasoning_details') and response_message.reasoning_details is not None}")
        print(f"  - Response (first 1000 chars):")
        print(f"{result[:1000]}...")
        if len(result) > 1000:
            print(f"[GROK DEBUG] ... (truncated, total {len(result)} chars)")
        print(f"\n[GROK DEBUG] Full response:")
        print(f"{result}")
        print(f"\n{'='*80}")
        print(f"[GROK DEBUG] {stage_name} - COMPLETE")
        print(f"{'='*80}\n")
        ...
        print(f"\n[GROK DEBUG] {stage_name} - API ERROR:")
        ...
        error_detail = str(e)
        print(f"\n[GROK DEBUG] {stage_name} - CONNECTION/TIMEOUT ERROR:")
        print(f"  Error type: {type(e).__name__}")
        ...
        print(f"\n[GROK DEBUG] {stage_name} - UNEXPECTED ERROR:")
        print(f"  Error type: {type(e).__name__}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error calling Grok: {str(e)}"
        )


@router.post("/analyze", response_model=GrokAnalysisResponse)
async def analyze_bonds_with_grok(
    ...
    """Анализирует облигации используя Grok 4.1 Fast через OpenRouter с многоэтапным pipeline.
    ...
    Returns:
        Объект GrokAnalysisResponse с результатами анализа, содержащий:
        - analysis: Финальный аналитический отчет (этап 5)
        - model_used: Название использованной модели
    ...
    Raises:
        HTTPException: Если произошла ошибка при чтении файлов (статус 500),
            если произошла ошибка при вызове API Grok (статус 502/503/504/500),
    ...
    Note:
        ...
        rate limit организации (если был предыдущий запрос). Reasoning mode
        используется для улучшения качества анализа. Все промежуточные этапы
        обрабатываются внутренне, пользователю возвращается только финальный отчет.
    """
    try:
        client = get_openai_client()
        
        print(f"[GROK Pipeline] Starting multi-stage analysis...")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"[GROK Pipeline] Stage 0: Saving uploaded files to temporary storage...")
            ...
            with open(bonds_path, "wb") as f:
                ...
                f.write(content)
            print(f"[GROK Pipeline] Bonds file saved: {len(content)} bytes")
            ...
            if has_forecast:
                ...
                with open(forecast_path, "wb") as f:
                    ...
                    f.write(content)
                print(f"[GROK Pipeline] Forecast file saved: {len(content)} bytes")
            ...
            if has_forecast:
                ...
                print(f"[GROK Pipeline] Stage 1 complete: forecast_summary length = {len(forecast_summary)} chars")
                previous_request_made = True
            else:
                print(f"[GROK Pipeline] Stage 1: SKIPPED (forecast file not provided)")
            
            # ========== ЭТАП 2: Анализ кривой бескупонной доходности (опционально) ==========
            yieldcurve_summary = ""
            if has_zerocupon:
                print(f"[GROK Pipeline] Stage 2: Processing zero-coupon yield curve...")
                ...
                yieldcurve_summary, conversation_messages = await call_grok_with_data(
                ...
                print(f"[GROK Pipeline] Stage 2 complete: yieldcurve_summary length = {len(yieldcurve_summary)} chars")
                previous_request_made = True
            else:
                print(f"[GROK Pipeline] Stage 2: SKIPPED (zerocupon file not provided)")
            
            # ========== ЭТАП 3: Нормализация данных по облигациям (всегда выполняется) ==========
            print(f"[GROK Pipeline] Stage 3: Normalizing bonds data...")
            ...
            bonds_normalized, conversation_messages = await call_grok_with_data(
                client, PROMPT_STAGE_3, bonds_json_str, model, "STAGE 3: Bonds Data Normalization",
                wait_before=previous_request_made,  # Пауза нужна, если был предыдущий запрос
                previous_messages=conversation_messages
            )
            print(f"[GROK Pipeline] Stage 3 complete: bonds_normalized length = {len(bonds_normalized)} chars")
            previous_request_made = True
            
            # ========== ЭТАП 4: Объединение макроданных (опционально, только если есть макроданные) ==========
            macro_overview = ""
            if has_forecast or has_zerocupon:
                print(f"[GROK Pipeline] Stage 4: Combining macro data...")
                macro_parts = []
                ...
                if macro_parts:
                    macro_data = "\n\n".join(macro_parts)
                    macro_overview, conversation_messages = await call_grok_with_data(
                        client, PROMPT_STAGE_4, macro_data, model, "STAGE 4: Macro Data Combination",
                        wait_before=previous_request_made,  # Пауза нужна, если был предыдущий запрос
                        previous_messages=conversation_messages
                    )
                    print(f"[GROK Pipeline] Stage 4 complete: macro_overview length = {len(macro_overview)} chars")
                    previous_request_made = True
                else:
                    print(f"[GROK Pipeline] Stage 4: SKIPPED (no macro data available)")
            else:
                print(f"[GROK Pipeline] Stage 4: SKIPPED (no macro data files provided)")
            
            # ========== ЭТАП 5: Итоговый аналитический отчёт ==========
            print(f"[GROK Pipeline] Stage 5: Generating final analytical report...")
            final_parts = []
            if macro_overview:
            ...
            final_report, _ = await call_grok_with_data(
                client, PROMPT_STAGE_5, final_data, model, "STAGE 5: Final Analytical Report",
                wait_before=previous_request_made,  # Пауза нужна, если был предыдущий запрос
                previous_messages=conversation_messages
            )
            print(f"[GROK Pipeline] Stage 5 complete: final_report length = {len(final_report)} chars")
            print(f"[GROK Pipeline] All stages complete. Returning final report to user.")
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"[GROK Pipeline] Unexpected error: {type(e).__name__}: {str(e)}")
        print(f"[GROK Pipeline] Traceback: {error_trace}")
        raise HTTPException(
            status_code=500,
            detail=f"Error during Grok analysis: {str(e)}"
        )
