"""Роутеры для работы с языковыми моделями OpenAI.

Этот модуль содержит роутеры FastAPI для обработки HTTP запросов на анализ
облигаций с использованием языковых моделей OpenAI (GPT). Реализует
...
class LLMAnalysisResponse(BaseModel):
    """Модель ответа для анализа облигаций через языковые модели OpenAI.
...
"""Глобальный экземпляр OpenAI клиента для работы с API OpenAI."""




def get_openai_client() -> openai.OpenAI:
    """Инициализирует и возвращает OpenAI клиент для работы с API OpenAI.
    
    Создает и настраивает OpenAI клиент для работы с API OpenAI, используя
    API ключ из настроек. Настраивает таймауты для длительных запросов к LLM
    (25 минут для чтения ответа) и количество повторных попыток.
    
    Returns:
        Экземпляр OpenAI клиента, настроенный для работы с API OpenAI.
    
    Raises:
        HTTPException: Если переменная окружения OPENAI_API_KEY не установлена
            (статус 500).
    
    Note:
        Клиент создается один раз и переиспользуется для всех запросов (singleton pattern).
        Таймаут чтения установлен в 25 минут для соответствия таймауту фронтенда
        (20 минут) с небольшим запасом.
    """
...
async def call_llm_with_data(client: openai.OpenAI, prompt: str, data_content: str, model: str = "gpt-5.1", stage_name: str = "Unknown") -> str:
    """Вызывает языковую модель OpenAI с промптом и данными.
    
    Выполняет запрос к языковой модели OpenAI с указанным промптом и данными.
    Для моделей GPT-5.x использует параметры reasoning_effort и verbosity для
    ...
    Args:
        ...
        model: Название модели для использования (по умолчанию "gpt-5.1").
    ...
    Raises:
        HTTPException: Если модель не вернула варианты ответа (статус 500),
            если ответ был обрезан из-за превышения лимита токенов (статус 500),
            если ответ был отфильтрован системой безопасности (статус 500),
            если произошла ошибка API OpenAI (статус 500),
            или если произошла другая ошибка (статус 500).
    
    Note:
        Максимальное количество токенов установлено в 32000 для длинных ответов.
        Для моделей GPT-5.x используются параметры reasoning_effort="high" и
        verbosity="high" для улучшения качества анализа.
    """
...
async def analyze_bonds(
    ...
    """Анализирует облигации используя языковую модель OpenAI с многоэтапным pipeline.
    ...
    Returns:
        Объект LLMAnalysisResponse с результатами анализа, содержащий:
        - analysis: Финальный аналитический отчет (этап 5)
        - model_used: Название использованной модели
    ...
    Raises:
        HTTPException: Если произошла ошибка при чтении файлов (статус 500),
            если произошла ошибка при вызове API OpenAI (статус 500),
            или если произошла другая ошибка во время анализа (статус 500).
    
    Note:
        Для моделей GPT-5.x используются параметры reasoning_effort="high" и
        verbosity="high" для улучшения качества анализа. Все промежуточные этапы
